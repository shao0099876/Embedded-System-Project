...
static sqlite3 *db;//全局的数据连接
static char* yjsql_debug;

/******在用户还没有创建table时可用该函数创建一个table****/
static int sqliteDB_create_oldtable(){
...
/******在用户还没有创建table时可用该函数创建一个table****/
static int sqliteDB_create_pathtable(){
 
	char* zErrMsg=NULL;
 	char sql[256]="CREATE TABLE path(name varchar(40) primary key,jidu varchar(40) not null,weidu varchar(40) not null,high varchar(40) not null,speed varchar(40) not null,timer varchar(100) primary key not null,flag varchar(1) not null);";
...
/******在用户还没有创建table时可用该函数创建一个table****/
static int sqliteDB_create_usertable(){
...
/******初始化数据库table时预插入两条数据****/
static int sqliteDB_insert_records()
{
 	char* zErrMsg=NULL;
...
/*** *建立与数据库的连接* **/
static int sqliteDB_open(){

	int rc;//操作标志
	//printf("开始连接数据库s\n");
	rc = sqlite3_open("/mnt/nfs/boa/YJDB.db", &db);
	if( rc ){
...
/**** 添加一条记录到已知或未知数据库表**/
static int sqliteDB_opt_addolduser(char *name,char *sex,char *age,char *address,char *state){
	int rc;
	char *zErrMsg = 0;
...
/**** 添加一条记录到已知或未知数据库表**/
static int sqliteDB_opt_adduser(char *name,char *sex,char *age,char *phone,char *address,char *relation,char *password){
	int rc;
	char *zErrMsg = 0;
	char *sql=0;//动态生成的SQL语句
...
/**** 添加一条记录到已知或未知数据库表**/
static int sqliteDB_opt_addpath(char *name,char *jidu,char *weidu,char *high,char *speed,char *timer,char *flag){
	int rc;
	char *zErrMsg = 0;
	char *sql=0;//动态生成的SQL语句
...
static int sqliteDB_opt_deleteolduser(char *name){
	int rc;
	char n[4];
	char *zErrMsg = 0;
	char *sql=0;//动态生成的SQL语句
...
/**** 删除表中的一条记录* @param name*/
static int sqliteDB_opt_deleteuser(char *name){
	int rc;
	char n[4];
	char *zErrMsg = 0;
	char *sql=0;//动态生成的SQL语句
...
/**** 删除表中的一条记录* @param name*/
static int sqliteDB_opt_deletepath(char *name){
	int rc;
	char n[4];
	char *zErrMsg = 0;
	char *sql=0;//动态生成的SQL语句
...
* //暂不提供修改函数2007-3-21
*/
static int sqliteDB_opt_modify();
/**
*
...
*/
 ...
 static char *sqliteDB_opt_select_allolduser(){
...
static char *sqliteDB_opt_select_alluser(){
...
static char *sqliteDB_opt_select_allpath(){//按照时间降序输出

	sqlite3_stmt* stmt=NULL;
 	char* zErrMsg=NULL;

	char dest[100000]={0,};//结果
	char s[]="no result";
...
static char **sqliteDB_opt_select_newpath_src(){//查询最新的path路径信息
	...
	char dest[1000]={0,};//结果
	char s[]="no result";
	char *tmp="SELECT * FROM path WHERE flag='0' ORDER BY timer DESC;";
 	int nret=sqlite3_prepare(db,tmp,strlen(tmp),&stmt,(const char**)(&zErrMsg));
 	char** res;
	if(nret!=SQLITE_OK){
...
static char *sqliteDB_opt_select_newpath(){//查询最新的path路径信息
...
static char *sqliteDB_opt_selectolduser(char *name){
...
static char *sqliteDB_opt_selectuser(char *name){
...
static char *sqliteDB_opt_selectpath(char *name){
...
static int sqliteDB_existolduser(char *name){
...
static int sqliteDB_existuser(char *name,char *password){
	...
	char *sql=0;//动态生成的SQL语句
	char tem_sql[256]="select * from user where name = '";//
	char tem_sql111[256]="and password = '";//
	char tem_sql0[5] = "'";
	char tem_sql1[5] = ";";

	sql = strcat(tem_sql,name);
	sql = strcat(sql,tem_sql0);
	sql=strcat(sql,tem_sql111);
   sql = strcat(sql,password);
   sql = strcat(sql,tem_sql0);
...
static int sqliteDB_existpath(char *name){
...
static double StringtoDouble(char *str){//将char*转换为double类型
	double f;
	f=atof(str);
	return f;
}

static char *DoubletoString(double f){//将double转换为char*类型
	char str[100];
	//sprintf(str,"%.5lf",f);
	return str;//返回数组首地址
}
...
static char *sqliteDB_opt_selectuserpassword(char *name){
...
static char* Login(char *name,char *password){//登录时进行的身份验证
	char *nam="no name";
	char *pas="wrong password";
	char *ses="successed";
	
	//先判断是否存在用户名
	int n=sqliteDB_existuser(name,password);//debug
	if(n==1){//不存在用户名,返回
		return nam;
	}
	else if(n==0){
